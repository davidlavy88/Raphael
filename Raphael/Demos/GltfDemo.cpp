...
// 4. Create swap chain and depth buffer
void GltfDemo::CreateSwapChainAndDepthBuffer()
{
...
void GltfDemo::CreateGeometry()
{
    ...
    for (size_t i = 0; i < vertexCount; ++i)
    {
        ...
        // Setting normals and texture coordinates to zero for now
    ...
    const UINT vertexBufferSize = static_cast<UINT>(vertices.size() * sizeof(VertexShaderInput));
    ...
    m_commandList->copyResource(m_vertexBuffer.get(), vertexUploadBuffer.get(), vertices.data(), vertexBufferSize);
    m_commandList->copyResource(m_indexBuffer.get(), indexUploadBuffer.get(), indices.data(), indexBufferSize);
    m_commandList->end();
...
LRESULT GltfDemo::HandleMessage(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
...
void GltfDemo::DestroyAppWindow()
{
...
LRESULT WINAPI GltfDemo::StaticWndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    // Retrieve the GltfDemo instance from window user data
    GltfDemo* app = nullptr;

    if (msg == WM_NCCREATE)
    {
        CREATESTRUCT* cs = reinterpret_cast<CREATESTRUCT*>(lParam);
        app = static_cast<GltfDemo*>(cs->lpCreateParams);
        ::SetWindowLongPtr(hWnd, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(app));
    }
    else
    {
        app = reinterpret_cast<GltfDemo*>(::GetWindowLongPtr(hWnd, GWLP_USERDATA));
    }
