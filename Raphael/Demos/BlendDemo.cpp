...
void BlendImGui::Display()
{
    ImGui::Begin("Blend Demo");
    ImGui::Text("Blend render");
    ImGui::End();
}

bool BlendDemo::Initialize(WindowInfo windowInfo)
{
...
void BlendDemo::CreateDescriptorHeaps()
{
...
// 4. Create swap chain and depth buffer
void BlendDemo::CreateSwapChainAndDepthBuffer(WindowInfo windowInfo)
{
...
void BlendDemo::CreateCommandObjects()
{
...
void BlendDemo::CreateGeometry()
{
	Mesh solidBox = MeshGenerator::CreateBox(8.0, 8.0, 8.0, 3);
	Mesh transparentBox = MeshGenerator::CreateBox(8.0, 8.0, 8.0, 3);

    Mesh fullMesh;
	fullMesh.m_vertices.reserve(solidBox.m_vertices.size() + transparentBox.m_vertices.size());
	fullMesh.m_vertices.insert(fullMesh.m_vertices.end(), solidBox.m_vertices.begin(), solidBox.m_vertices.end());
	fullMesh.m_vertices.insert(fullMesh.m_vertices.end(), transparentBox.m_vertices.begin(), transparentBox.m_vertices.end());
	fullMesh.m_drawMeshes["solidBox"] = { 0, 0, static_cast<uint32_t>(solidBox.m_indices16.size()) };
	fullMesh.m_indices16.reserve(solidBox.m_indices16.size() + transparentBox.m_indices16.size());
	fullMesh.m_indices16.insert(fullMesh.m_indices16.end(), solidBox.m_indices16.begin(), solidBox.m_indices16.end());
	fullMesh.m_indices16.insert(fullMesh.m_indices16.end(), transparentBox.m_indices16.begin(), transparentBox.m_indices16.end());
	fullMesh.m_drawMeshes["transparentBox"] = { 
		static_cast<uint32_t>(solidBox.m_vertices.size()), // vertex buffer offset for transparent box starts after solid box vertices
		static_cast<uint32_t>(solidBox.m_indices16.size()), // index buffer offset for transparent box starts after solid box indices
		static_cast<uint32_t>(transparentBox.m_indices16.size()) }; // index count for transparent box
	std::vector<VertexWithTexCoord> totalVertices(solidBox.m_vertices.size() + transparentBox.m_vertices.size());

    const UINT vertexBufferSize = static_cast<UINT>(fullMesh.m_vertices.size() * sizeof(VertexWithTexCoord));
    const UINT indexBufferSize = static_cast<UINT>(fullMesh.m_indices16.size() * sizeof(uint16_t));
    m_indexCount = static_cast<UINT>(fullMesh.m_indices16.size());
    ...
    {
        memcpy(vertexData, fullMesh.m_vertices.data(), vertexBufferSize);
    ...
    {
        memcpy(indexData, fullMesh.m_indices16.data(), indexBufferSize);
    ...
    m_commandList->copyResource(m_vertexBuffer.get(), vertexUploadBuffer.get(), fullMesh.m_vertices.data(), vertexBufferSize);
    m_commandList->copyResource(m_indexBuffer.get(), indexUploadBuffer.get(), fullMesh.m_indices16.data(), indexBufferSize);
    m_commandList->end();
...
void BlendDemo::CreateConstantBuffers()
{
    for (UINT i = 0; i < g_frameCount; i++)
    {
...
void BlendDemo::CreateRootSignature()
{
...
void BlendDemo::UpdateConstantBuffers()
{
    // Rotate the boxes slowly around the Y axis
...
void BlendDemo::Shutdown()
{
    ...
    OutputDebugStringA("Shutting down BlendDemo and releasing resources.\n");
}

void BlendDemo::Resize(unsigned int width, unsigned int height)
{
