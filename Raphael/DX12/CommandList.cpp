...
namespace raphael
{
    ...
    void CommandList::transitionResource(ResourceDx12* resource, ResourceBindFlags bindFlags)
    {
        ...
        CD3DX12_RESOURCE_BARRIER barrier = CD3DX12_RESOURCE_BARRIER::Transition(resource->getNativeResource(), beforeState, afterState);
    ...
    void CommandList::setRenderTargets(uint32_t numRenderTargets, const D3D12_CPU_DESCRIPTOR_HANDLE* rtvHandles, const D3D12_CPU_DESCRIPTOR_HANDLE* dsvHandle)
    {
        m_commandList->OMSetRenderTargets(numRenderTargets, rtvHandles, true, dsvHandle);
    }

    void CommandList::clearAndSetRenderTargets(const RenderPassDesc& renderPassDesc)
    {
        ...
        for (UINT i = 0; i < renderPassDesc.numRenderTargets; ++i)
        {
            ...
            clearRenderTargetView(rtvHandles[i], renderPassDesc.clearColor);
        }
        if (renderPassDesc.hasDepthStencil)
        {
            clearDepthStencilView(renderPassDesc.dsvHandle.cpuHandle, renderPassDesc.clearDepth, renderPassDesc.clearStencil);
        }

        // Set render targets for the output merger stage
        setRenderTargets(renderPassDesc.numRenderTargets, rtvHandles, renderPassDesc.hasDepthStencil ? &renderPassDesc.dsvHandle.cpuHandle : nullptr);
    }
